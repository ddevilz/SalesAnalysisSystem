...
public class EwayServiceImpl implements EwayService{
    ...
    @Override
    public Eway findEwayWithInvoiceId(Long id) {
        Optional<Eway> ewayDb = ewayRepository.findByInvoiceId(id);
        System.out.println(ewayDb.get().toString());
        if (ewayDb.isEmpty()) {
            throw new ResourceNotFoundException("No Eways found with " + id );
        }
        return ewayDb.get();
    }
